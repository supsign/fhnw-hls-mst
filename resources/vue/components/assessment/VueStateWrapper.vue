<template>
    <div v-if="!!assessment"
         class="rounded border shadow-md fixed bottom-0 left-0 mt-2 w-full bg-hls-200">
        <div
        ...
        >
            <vue-assessment-state ref="assessmentState"
        ...
        <div v-if="!!specialization"
             ...
             class="border-b text-sm lg:text-base p-3 bg-hls transition duration-200 ease-in-out overflow-y-scroll max-h-56 lg:overflow-y-auto lg:max-h-full"
        >
            <vue-specialization-state ref="specializationState"
                                      :completions="completions"
                                      :planning="planning"
                                      :planning-id="planningId"
                                      :semesters="semesters"
        ...
        <div v-if="!!crossQualification"
             ...
             class="border-b text-sm lg:text-base p-3 bg-hls transition duration-200 ease-in-out overflow-y-scroll max-h-56 lg:overflow-y-auto lg:max-h-full"
        >
            <vue-cross-qualification-state ref="crossQualificationState"
                                           :completions="completions"
                                           :cross-qualification="crossQualification"
        ...
        <div class="md:w-full mx-auto grid grid-cols-3 lg:h-16">
            <div
                class="text-center border-l border-hls text-sm my-auto h-full lg:flex lg:justify-center lg:items-center lg:space-x-2"
                @click="toggleShowCrossQualification">
                <div>{{ countCredits }}&nbsp;|&nbsp;180</div>
                <div>ECTS</div>
            </div>
            <div v-if="!specialization && !crossQualification"></div>
            <div v-if="!!crossQualification"
                 class="text-center border-l border-hls text-sm hover:bg-hls hover:border-gray-200 my-auto cursor-pointer lg:h-full lg:flex lg:justify-center lg:items-center lg:space-x-2"
                 @click="toggleShowCrossQualification">
            ...
            <div v-if="!!specialization"
                 class="text-center border-l border-hls text-sm hover:bg-hls hover:border-gray-200 my-auto cursor-pointer lg:h-full lg:flex lg:justify-center lg:items-center lg:space-x-2"
                 @click="toggleShowSpecialization">
