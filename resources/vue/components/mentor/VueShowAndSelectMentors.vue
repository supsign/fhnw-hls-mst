    ...
    <div class="p-2 rounded bg-white flex flex-col shadow-xl">
        <div class="divide-y p-3 text-sm md:text-base flex-grow">
            <div class="pb-2 flex flex-row justify-between">
                ...
                <div>
                    <button v-if="studyField" class="button-primary" @click="approve">freigeben</button>
                </div>
            </div>
            <div>
                ...
                <div v-if="!studyField">
                    Bitte wenden sie sich an die Administration, um Planungen für Stgl freizugeben.
                </div>
...
export default class VueShowAndSelectMentors extends BaseComponent {
    ...
    public get availableMentors() {
        return this.allMentors.filter(
            (mentor) => !this.mentorStudents.find((mentorStudent) => mentor.id === mentorStudent.mentor_id)
        );
    }
    ...
    public saveSelectedMentor(mentor: IMentor) {
        axios
            .post<IMentorStudent>(`/webapi/mentor/${mentor.id}/students/${this.studentId}`)
            .then((res) => {
    ...
    public approve() {
        ...
        const mentors = this.availableMentors.filter((mentor) => {
    ...
    public remove(mentorStudent: IMentorStudent) {
        const index = this.mentorStudents.findIndex(
            (mentorStudentAttached) => mentorStudentAttached.id === mentorStudent.id
        );
        if (index === -1) {
            return;
        }
        this.mentorStudents.splice(index, 1);
        axios
            .delete(`/webapi/mentor/${mentorStudent.mentor_id}/students/${mentorStudent.student_id}`)
