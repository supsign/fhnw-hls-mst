...
export const byId = (state: IBaseState<IModel>) => (id: number): IModel => {
...
export const filter = (state: IBaseState<IModel>) => (
    filter: (value: IModel, index: number, array: IModel[]) => unknown
): Array<IModel> => {
    return state.entities.local.filter(filter);
};

export const entityIsLoading = (state: IBaseState<IModel>) => (
    id: number
): boolean => {
