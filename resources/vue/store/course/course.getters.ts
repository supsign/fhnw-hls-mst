...
export const getByAcquisitionSkillIds =
    (state: IBaseState<ICourse>) =>
    (skillIds: number[]): ICourse[] => {
        const courses = state.entities.local.filter((entity) => {
            return entity.course_skills.find((courseSkill) => {
