...
export const getCoursePlanning =
    (state: IBaseState<ICoursePlanning>) =>
    (planningId: number, courseId: number): ICoursePlanning | null => {
        return state.entities.local.find(
            (entity) => entity.course_id === courseId && entity.planning_id === planningId
        );
    };

export const byPlanningId =
    (state: IBaseState<ICoursePlanning>) =>
